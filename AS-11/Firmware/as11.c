...
#include "../Software/Bootloader/boot_data.h"
...
void main() {
	...
	reg_uart_enable = 0;
	configure_io_programming();
    reg_wb_enable = 1;
    reg_mprj_proj_sel = 0b0100101;
    reg_mprj_settings = 0x8000C;
	...
	for(uint32_t i = 0; i < pgm_len; i++) {
		uint32_t base = ((i & 0xFFFF) << 6) | OEB | WEB | HALTD;
		reg_mprj_datal = base | ALE;
		asm volatile("nop");
		reg_mprj_datal = base;
		base = (pgm[i] << 5) | OEB | HALTD;
		reg_mprj_datal = base | WEB;
		asm volatile("nop");
		reg_mprj_datal = base;
		asm volatile("nop");
		reg_mprj_datal = base | WEB;
